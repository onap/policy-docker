...
releases=$(find /home/policy/sql/*/*/downgrade/*.* -type f -name *.sql -exec dirname {} \; | sort -u | rev | cut -f1-3 -d/ | rev)
for release in $releases
do
        name=$(echo $release | cut -f1 -d/)
        version=$(echo $release | cut -f2 -d/)
        operation=$(echo $release | cut -f3 -d/)
        ...
        find $POLICY_HOME/etc/db/migration/${SCHEMA}/sql/$name/$version/$operation/* \
                -name *.sql -exec echo \\. {} \; | nl | sort -n | cut -f 2- \
                >> $POLICY_HOME/etc/db/migration/${SCHEMA}/sql/$version-$name.$operation.sql
done
echo "Done"
