...
# Create downgrade directories
cd /home/policy/sql && find . -type d -not -path '*/*/upgrade/*' -not -path '*/*/upgrade' \
        -exec mkdir -p $POLICY_HOME/etc/db/migration/${SCHEMA}/sql/{} \;
# Copy file to downgrade directories
cd /home/policy/sql && find . -type f  -not -path '*/*/upgrade/*' -not -path '*/*/upgrade'  \
...
releases=$(find $POLICY_HOME/etc/db/migration/${SCHEMA}/sql/*/*/downgrade -type d | sort -u | rev | cut -f2-3 -d/ | rev)
