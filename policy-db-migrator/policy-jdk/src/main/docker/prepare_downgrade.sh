...
releases=$(find /home/policy/sql/*/downgrade/*.* -type f -name *.sql | sed 's/-.*//' | sort -u | rev | cut -f1-3 -d/ | rev)
...
do
        ...
        find $POLICY_HOME/etc/db/migration/${SCHEMA}/sql/*/downgrade/ \
                -name ${version}*.sql -exec echo \\. {} \; | sort | nl | sort -nr | cut -f 2- \
                >> $POLICY_HOME/etc/db/migration/${SCHEMA}/sql/$version-$name.$operation.sql
done
echo "Done"
