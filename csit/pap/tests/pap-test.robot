...
GetReq
    ...
    ${resp}=  PerformGetRequest  localhost  ${POLICY_PAP_PORT}  ${url}  200  null  ${auth}
    [return]  ${resp}
...
Metrics
    ...
    ${auth}=  PolicyAdminAuth
    ${resp}=  GetMetrics  localhost  ${POLICY_PAP_PORT}  ${auth}  /policy/pap/v1/
...
AddPdpGroup
    ...
    ${auth}=  PolicyAdminAuth
    PerformPostRequest  localhost  ${POLICY_PAP_PORT}  /policy/pap/v1/pdps/groups/batch  200  ${postjson}  null  ${auth}
...
ActivatePdpGroup
    ...
    ${auth}=  PolicyAdminAuth
    PerformPutRequest  localhost  ${POLICY_PAP_PORT}  /policy/pap/v1/pdps/groups/testGroup  200  state=ACTIVE  ${auth}
...
DeployPdpGroups
    ...
    ${auth}=  PolicyAdminAuth
    PerformPostRequest  localhost  ${POLICY_PAP_PORT}  /policy/pap/v1/pdps/deployments/batch  202  ${postjson}  null  ${auth}
...
UndeployPolicy
    ...
    ${auth}=  PolicyAdminAuth
    PerformDeleteRequest  localhost  ${POLICY_PAP_PORT}  /policy/pap/v1/pdps/policies/onap.restart.tca  202  ${auth}

UndeployPolicyWithMetadataSet
    [Documentation]  Undeploy a policy named 'operational.apex.decisionMaker' from PdpGroups
    ${auth}=  PolicyAdminAuth
    PerformDeleteRequest  localhost  ${POLICY_PAP_PORT}  /policy/pap/v1/pdps/policies/operational.apex.decisionMaker  202  ${auth}
...
DeactivatePdpGroup
    ...
    ${auth}=  PolicyAdminAuth
    PerformPutRequest  localhost  ${POLICY_PAP_PORT}  /policy/pap/v1/pdps/groups/testGroup  200  state=PASSIVE  ${auth}

DeletePdpGroups
    [Documentation]  Delete the PdpGroup named 'testGroup' from policy database
    ${auth}=  PolicyAdminAuth
    PerformDeleteRequest  localhost  ${POLICY_PAP_PORT}  /policy/pap/v1/pdps/groups/testGroup  200  ${auth}
