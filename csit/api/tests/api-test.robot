...
Metrics
    ...
    ${resp}=  GetMetrics  localhost  ${POLICY_API_PORT}  ${auth}  /policy/api/v1/
...
GetReq
     [Arguments]  ${url}
     ${auth}=  PolicyAdminAuth
     ${resp}=  PerformGetRequest  localhost  ${POLICY_API_PORT}  ${url}  200  null  ${auth}
     [return]  ${resp}

DeleteReq
     [Arguments]  ${url}  ${expectedstatus}
     ${auth}=  PolicyAdminAuth
     ${resp}=  PerformDeleteRequest  localhost  ${POLICY_API_PORT}  ${url}  ${expectedstatus}  ${auth}
     [return]  ${resp}
...
CreatePolicyType
     ...
     ${auth}=  PolicyAdminAuth
     ${resp}=  PerformPostRequest  localhost  ${POLICY_API_PORT}  ${url}  ${expectedstatus}  ${postjson}  null  ${auth}
