...
PerformPostRequest
    [Arguments]  ${hostname}  ${port}  ${url}  ${expectedstatus}  ${postjson}  ${params}  ${auth}
...
PerformPutRequest
    [Arguments]  ${hostname}  ${port}  ${url}  ${expectedstatus}  ${params}  ${auth}
    Log  Creating session http://${hostname}:${port}
    ${session}=  Create Session  policy  http://${hostname}:${port}  auth=${auth}
    ${headers}=  Create Dictionary  Accept=application/json  Content-Type=application/json
...
PerformDeleteRequest
    [Arguments]  ${hostname}  ${port}  ${url}  ${expectedstatus}  ${auth}
    Log  Creating session http://${hostname}:${port}
    ${session}=  Create Session  policy  http://${hostname}:${port}  auth=${auth}
    ${headers}=  Create Dictionary  Accept=application/json  Content-Type=application/json
...
CreatePolicy
    ...
    ${resp}=  PerformPostRequest  localhost  ${POLICY_API_PORT}  ${url}  ${expectedstatus}  ${postjson}  null  ${policyadmin}
...
CreateNodeTemplate
    ...
    ${policyadmin}=  PolicyAdminAuth
    ${resp}=  PerformPostRequest  localhost  ${POLICY_API_PORT}  ${url}  ${expectedstatus}  ${postjson}  \  ${policyadmin}
...
QueryPdpGroups
    ...
    ${policyadmin}=  PolicyAdminAuth
    ${resp}=  PerformGetRequest  localhost  ${POLICY_PAP_PORT}  /policy/pap/v1/pdps  200  null  ${policyadmin}
...
QueryPolicyAudit
    ...
    ${policyadmin}=  PolicyAdminAuth
    ${resp}=  PerformGetRequest  localhost  ${POLICY_PAP_PORT}  ${url}  ${expectedstatus}  recordCount=2   ${policyadmin}
...
QueryPolicyStatus
    ...
    ${policyadmin}=  PolicyAdminAuth
    ${resp}=  PerformGetRequest  localhost  ${POLICY_PAP_PORT}  /policy/pap/v1/policies/status  200  null   ${policyadmin}
    FOR    ${responseEntry}    IN    @{resp.json()}
    Exit For Loop IF      '${responseEntry['policy']['name']}'=='${policyName}'
    END
...
GetMetrics
    [Arguments]  ${hostname}  ${port}  ${auth}  ${context_path}
    Log  Creating session http://${hostname}:${port}
    ${session}=  Create Session  policy  http://${hostname}:${port}  auth=${auth}
