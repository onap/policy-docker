...
DeployPolicies
    ...
    PerformPostRequest  localhost  ${POLICY_PAP_PORT}  /policy/pap/v1/pdps/policies  202  ${postjson}  null  ${policyadmin}
...
UndeployMonitorPolicy
    ...
    ${policyadmin}=  PolicyAdminAuth
    PerformDeleteRequest  localhost  ${POLICY_PAP_PORT}  /policy/pap/v1/pdps/policies/onap.restart.tca  202  ${policyadmin}
...
PdpxGetReq
    ...
    ${resp}=  PerformGetRequest  localhost  ${POLICY_PDPX_PORT}  ${url}  200  null  ${hcauth}
    [return]  ${resp}

DecisionPostReq
    [Arguments]  ${postjson}  ${abbr}
    ${hcauth}=  HealthCheckAuth
    ${resp}=  PerformPostRequest  localhost  ${POLICY_PDPX_PORT}  /policy/pdpx/v1/decision  200  ${postjson}  ${abbr}  ${hcauth}
    [return]  ${resp}
