...
for image in ${IMAGES}; do
    ...
    TAGS="latest ${release_version} ${release_version}-SNAPSHOT-latest ${release_version}-STAGING-latest"
    for tag in ${TAGS}; do
    ...
    # Create timestamped multiarch tag
    ./manifest-tool push from-args \
        --ignore-missing \
        --platforms "linux/${ARCHES// /,linux/}" \
        --template "${image}:${release_version}-ARCH" \
        --target "${image}:${release_version}-${TIMESTAMP}"
done
