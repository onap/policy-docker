...
echo "Create BRMSParamCCVPNBandwidthOnDemand Policy"
curl -k -v --silent -X PUT --header 'Content-Type: application/json' --header 'Accept: text/html' --header 'ClientAuth: cHl0aG9uOnRlc3Q=' --header 'Authorization: Basic dGVzdHBkcDphbHBoYTEyMw==' --header 'Environment: TEST' -d '{
    "policyConfigType": "BRMS_PARAM",
    "policyName": "com.BRMSParamCCVPNBandwidthOnDemand",
    "policyDescription": "BRMS Param CCVPN Bandwidth on demand policy",
    "policyScope": "com",
    "attributes": {
        "MATCHING": {
            "controller" : "amsterdam"
...
echo "pushPolicy : PUT : com.BRMSParamCCVPNBandwidthOnDemand"
curl -k -v --silent -X PUT --header 'Content-Type: application/json' --header 'Accept: text/plain' --header 'ClientAuth: cHl0aG9uOnRlc3Q=' --header 'Authorization: Basic dGVzdHBkcDphbHBoYTEyMw==' --header 'Environment: TEST' -d '{
  "pdpGroup": "default",
  "policyName": "com.BRMSParamCCVPNBandwidthOnDemand",
  "policyType": "BRMS_Param"
